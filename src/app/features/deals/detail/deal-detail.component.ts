...
@Component({
  ...
  standalone: true,
  imports: [CurrencyPipe, DatePipe, RouterLink, BadgeComponent, ConfirmDialogComponent, LoadingSpinnerComponent],
  template: `
      ...
      @if (loading()) {
        ...
        <div class="flex items-start justify-between">
          ...
          <div class="flex gap-2">
            <a [routerLink]="['/deals', deal()!.id, 'edit']" class="btn-secondary">Editar</a>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div class="card">
            <h2 class="text-base font-semibold text-surface-100 mb-4">Información del deal</h2>
            <dl class="space-y-3">
              <div>
                <dt class="text-xs text-surface-500 uppercase tracking-wider mb-1">Valor</dt>
                <dd class="text-xl font-bold text-green-400">
                  {{ deal()!.value != null ? (deal()!.value! | currency: deal()!.currency) : '—' }}
                </dd>
              </div>
              <div>
                <dt class="text-xs text-surface-500 uppercase tracking-wider mb-1">Contacto</dt>
                <dd class="text-sm text-surface-200">
                  @if (deal()!.contact) {
                    <a [routerLink]="['/contacts', deal()!.contact!.id]" class="text-primary-400 hover:text-primary-300">
              ...
              <div>
                ...
                <dd class="text-sm text-surface-200">
                  {{ deal()!.assigned_to ? deal()!.assigned_to!.first_name + ' ' + deal()!.assigned_to!.last_name : '—' }}
                </dd>
              </div>
              <div>
                <dt class="text-xs text-surface-500 uppercase tracking-wider mb-1">Cierre estimado</dt>
                <dd class="text-sm text-surface-200">
          ...
          <!-- Stage history -->
          <div class="card">
            <h2 class="text-base font-semibold text-surface-100 mb-4">Historial de etapas</h2>
            @if (deal()!.stage_history.length === 0) {
              <p class="text-sm text-surface-500">Sin historial de etapas</p>
            } @else {
              <div class="space-y-3">
                    ...
                    <div>
                      ...
                      <p class="text-xs text-surface-500">
                        ...
                        @if (entry.exited_at) {
                          ...
                          <span class="text-primary-400">(actual)</span>
                        }
                      </p>
                    </div>
                  </div>
    ...
    <app-confirm-dialog
      ...
      title="Eliminar deal"
      message="¿Estás seguro de que deseas eliminar este deal? Esta acción no se puede deshacer."
      (confirm)="deleteDeal()"
